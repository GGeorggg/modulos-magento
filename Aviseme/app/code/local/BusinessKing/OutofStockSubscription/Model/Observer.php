...
class BusinessKing_OutofStockSubscription_Model_Observer
{
	...
	public function checkStock($observer)
	{
		...
		if ($product) {
		    ...
		    if ($isInStock>=1) {
		    	$subscriptions = Mage::getResourceModel('outofstocksubscription/info')->getSubscriptions($product->getData('product_id'));
		    	if (count($subscriptions) > 0) {

		    		$product = Mage::getModel('catalog/product')->load($product->getData('product_id'));
