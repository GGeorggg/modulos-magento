...
class Bullmkt_Correios_Model_Carrier_CorreiosMethod
    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    {       
        ...
        // Check ZIP Code
        if(!preg_match("/^([0-9]{8})$/", $this->_toZip) && $this->_toZip)
        {
            // Invalid Zip Code
    ...
    protected function _inicialCheck(Mage_Shipping_Model_Rate_Request $request){
        ...
        {
            //Disabled
            Mage::log('Bullmkt_Correios: Disabled');
            return false;
        }
        ...
        if ($origCountry != "BR" || $destCountry != "BR"){
            ...
            Mage::log('Bullmkt_Correios: Out of delivery area');
            return false;
        }
        ...
        if(!preg_match("/^([0-9]{8})$/", $this->_fromZip)){
            //From zip code error
            Mage::log('Bullmkt_Correios: From ZIP Code Error');
            return false;
        }
    ...
    protected function _getCorreiosReturn(){
        ...
        try {
            ...
            if ($content == ""){
                ...
                return $this->_correiosOffline(intval($this->_toZip), $peso);
            }
	...
	protected function _sendNotifyEmail()
	{
		$emailTemplate  = Mage::getModel('core/email_template')
            ->loadDefault('correios');
        $emailTemplate
            ...
            ->setTemplateSubject('Template to notify administrator that the correios is gone away');
        $result = $emailTemplate->send(Mage::getStoreConfig('trans_email/ident_general/email'),Mage::getStoreConfig('trans_email/ident_general/name'),null);		
	}
	
	
	protected function _correiosOffline($cep_destino, $peso)
	{
		$fields = array('peso','valor','prazo_entrega' => 'prazo','codigo' => 'servico','cep_destino_ini','cep_destino_fim');
		
		$collection = Mage::getModel('correios/correios')
			->getCollection()
			->addFieldToSelect($fields)
			->addFieldToFilter('cep_destino_ini',array('lteq' => $cep_destino))
			->addFieldToFilter('cep_destino_fim',array('gteq' => $cep_destino))
			->addFieldToFilter('peso',array('eq' => $peso));
