 ...
 */
class Uni_Banner_Adminhtml_GroupController extends Mage_Adminhtml_Controller_action {
    ...
    public function groupgridAction() {
        ...
        $this->getResponse()->setBody(
                $this->getLayout()->createBlock('banner/adminhtml_group_edit_tab_group')->toHtml()
        );
    }
	
	public function effectsgridAction() {
		$this->_initAction();
        $this->getResponse()->setBody(
                $this->getLayout()->createBlock('banner/adminhtml_group_edit_tab_group')->toHtml()
        );
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $model = Mage::getModel('banner/group')->load($this->getRequest()->getParam('id'));
        ...
        if (!is_array($bannerIds)) {
            ...
            try {
                foreach ($bannerIds as $bannerId) {
                    $banner = Mage::getModel('banner/group')->load($bannerId);
    ...
    public function massStatusAction() {
        ...
        if (!is_array($bannerIds)) {
            ...
            try {
                foreach ($bannerIds as $bannerId) {
                    $banner = Mage::getSingleton('banner/group')
                                    ->load($bannerId)
    ...
    public function exportCsvAction() {
        $fileName = 'banner.csv';
        $content = $this->getLayout()->createBlock('banner/adminhtml_group_grid')
                        ->getCsv();
