 ...
 */
class Uni_Banner_Adminhtml_ImagesController extends Mage_Adminhtml_Controller_action {
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('banner/images')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('banner/adminhtml_images_edit'))
                    ->_addLeft($this->getLayout()->createBlock('banner/adminhtml_images_edit_tabs'));
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            $model = Mage::getModel('banner/images');
            $model->setData($data)
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $model = Mage::getModel('banner/images')->load($this->getRequest()->getParam('id'));
                $_helper = Mage::helper('banner');
        ...
        if (!is_array($bannerIds)) {
            ...
            try {
                foreach ($bannerIds as $bannerId) {
                    $model = Mage::getModel('banner/images')->load($bannerId);
                    $_helper = Mage::helper('banner');
                    $filePath = Mage::getBaseDir('media').DS.$_helper->updateDirSepereator($model->getFilename());
                    $model->delete();
                    $this->removeFile($filePath);
    ...
    public function massStatusAction() {
        ...
        if (!is_array($bannerIds)) {
            ...
            try {
                foreach ($bannerIds as $bannerId) {
                    $banner = Mage::getSingleton('banner/images')
                                    ->load($bannerId)
    ...
    public function exportCsvAction() {
        $fileName = 'images.csv';
        $content = $this->getLayout()->createBlock('banner/adminhtml_images_grid')
                        ->getCsv();
